...
def load_image_file(file, mode='RGB'):
    img = imageio.imread(file, pilmode=mode)

    # Resize the image if it's very large
    max_height = 800
    if img.shape[0] > max_height or img.shape[1] > max_height:
        pil_img = Image.fromarray(img)
        pil_img.thumbnail((max_height, max_height), Image.LANCZOS)
