...
y = np.array(full_data[:, -1]).astype(int)  # Ensure labels are integers
...
for image_path in get_prediction_images(prediction_dir):
    ...
    predictions = [(le.inverse_transform([int(pred)])[0].title(), loc) if rec else ("Unknown", loc) for pred, loc, rec in
